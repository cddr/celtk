...
(defun do-execute (program args &optional (wt nil))
  ...
  (let ((fullstring program))
    ...
    #+not (let ((proc (excl:run-shell-command
			    #+:mswindows fullstring
			    #-:mswindows (apply #'vector program program args)
...
(defun init-wish ()
  ...
  (send-wish "proc callbackval {s value} {puts \"(:callback \\\"$s\\\" $value)\"} ")
      ...
      (or
       (let ((event (pop (wish-event-queue *wish*))))
        ...
        (if (or blocking (can-read (wish-stream *wish*)))
            (let ((event (read-preserving-whitespace (wish-stream *wish*) nil nil)))
              ;;(when event (ukt:trc "read-event > read" event))
              event)
          no-event-value))
    (reader-error (c)
      (let ((d (read-line (wish-stream *wish*))))
        (print `(read-event-baddata ,d ,c))
        nil))))

(defun peek-char-no-hang (stream)
  (and (listen stream) (peek-char t stream nil nil)))
...
(defun read-data ()
  ...
  (loop for d = (read-wish)
        ;;do (print `(<tk ,d))
      unless (listp d)
      do (break "read-data > unexpected Tk output: ~a ~a~%" d (read-all (wish-stream *wish*)))
      if (eq (first d) :data)
      do (return (second d))
      else
      do (setf (wish-event-queue *wish*)
                 (nconc (wish-event-queue *wish*) (list d)))))
...
(defun add-callback (sym fun)
  ...
  ;;#+nah (when *debug-tk*
  (assert fun () "No function supplied for callback ~a" sym)
  ;;(format t "~&add-callback > :sym ~A ) ~a~%" sym fun)
...
(defun callback (sym args)
  "perform the call of the function associated with sym and the args arg"
  ;; (ukt::trc "handling callback" sym args)

  (multiple-value-bind (fun present-p)
      (gethash sym (wish-callbacks *wish*))
    (cond
     (fun (apply fun args))
     (present-p (break "callback ~a was registered with null function" sym))
     (t
      (loop for cb being the hash-keys of (wish-callbacks *wish*)
          when (string-equal (symbol-name sym) (symbol-name cb))
          do
            (format t "~&missing callback ~a ~a ~s" sym (symbol-package sym) (symbol-name sym))
            (format t "~&found match (by name) of missing callback ~a ~a ~s" cb (symbol-package cb) (symbol-name cb) (symbol-name cb))
          finally
            (break "callback unknown ~a, pkg ~a args ~a" sym (symbol-package sym) args))))))
...
(defun process-one-event (event)
  ;; (ukt::trc nil "POE> entry" event (not (listp event)))
  (when event
    (when *debug-tk*
    ...
    (cond
     ...
     ((not (listp event))
      (when *trace-tk*
        ...
        (finish-output *trace-output*))
      (print `(bad-line> ,event ,(read-line (wish-stream *wish*))) *trace-output*)
      (cells:c-break "error from tk"))
...
(defun ltktest()
  (with-ltk ()
    (let* ((bar (make-instance 'frame))
           (kt1 (make-instance 'radio-button :master bar :text "Yes" :value "Yes" :variable "kwt"))
           (kt2 (make-instance 'radio-button :master bar :text "No" :value "No" :variable "kwt"))
           (fr (make-instance 'frame :master bar))
           (lr (make-instance 'label :master fr :text "Rottion:"))
      ...
      (pack kt1 :side :left)
      (pack kt2 :side :left)
      (print `(:kt2-command ,(init-command kt2)))
      (setf (value kt2) "yes")
      (pack fr :side :left)
      (pack lr :side :left)
