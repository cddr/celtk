...
(defparameter +keysym-code+ "space                               32     0x0020
exclam                              33     0x0021
quotedbl                            34     0x0022
...
BackSpace                         0008     0x0008
Tab                               0009     0x0009
Delete                            0127     0x007f")

(defparameter *keysym-char*
  (loop with ht = (make-hash-table :test 'eq)
      for line in (ukt:split-sequence #\Newline +keysym-code+)
      for (ksym dec nil) = (ukt:split-sequence #\space line :remove-empty-subseqs t)
      do ;;(print (list (intern ksym :celtk) (code-char (parse-integer dec))))
        (setf (gethash (intern ksym :celtk) ht) (code-char (parse-integer dec)))
      finally (return ht)))

(defparameter *char-keysym*
  (loop with v = (make-array 4000 :element-type 'fixnum)
      for line in (ukt:split-sequence #\Newline +keysym-code+)
      for (ksym dec nil) = (ukt:split-sequence #\space line :remove-empty-subseqs t)
      do (setf (aref v (parse-integer dec)) ksym)
      finally (return v)))

(ukt:export! keysym-char)

(defun keysym-char (keysym$)
  (gethash (intern keysym$ :celtk) *keysym-char*))

#+test
(gethash '|radical| *keysym-char*)

(defun char-keysym (char)
  (aref *char-keysym* (char-code char)))

(defun char-code-keysym (char-code)
  (aref *char-keysym* char-code))
...
;;;(defparameter +keysym-chars+ '((minus . #\-)
;;;                               (period . #\.)
;;;                               (asciicircum . #\^)
;;;                               (plus . #\+)
;;;                               (space . #\space)
...
(export! keysym-char keysym-sym minus period asciicircum plus backspace
          ...
          slash backslash asterisk at tab)
;;;
;;;(defun tk-keysym-to-char (keysym)
;;;  (cdr (assoc keysym +keysym-chars+)))
;;;
;;;(defun keysym-char (keysym$)
;;;  (or (tk-keysym-to-char (keysym-sym keysym$))
;;;    (when (= 1 (length keysym$))
;;;      (schar keysym$ 0))))
          
(defun keysym-sym (keysym$)
  (read-from-string (string-upcase keysym$)))